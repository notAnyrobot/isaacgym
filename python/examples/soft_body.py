...
print("Loading asset '%s' from '%s'" % (soft_asset_file, ASSET_PATH))
soft_asset = gym.load_asset(sim, ASSET_PATH, soft_asset_file, asset_options)
...
for i in range(num_envs):
    ...
    for j in range(asset_soft_body_count):
        ...
        actor_soft_materials[j].youngs = random.uniform(
            youngs * 0.2, youngs * 2.4
        )
    ...
    gym.set_joint_target_position(
        env, gym.get_joint_handle(env, "soft", "rail"), 0.0
    )
